...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$client_id || !$status || empty($product_ids)) {
        ...
        $stmt = mysqli_prepare($conn, "UPDATE orders SET client_id = ?, status = ? WHERE id = ?");
        mysqli_stmt_bind_param($stmt, 'isi', $client_id, $status, $order_id);
        mysqli_stmt_execute($stmt);

        // Очищення старих товарів
        ...
        log_event($conn, 'редагування', 'замовлення', $order_id, "Оновлено замовлення: клієнт #$client_id, статус '$status'");
        set_flash_message("✅ Замовлення оновлено");
        header("Location: ../../index.php#orders");
        exit;
    }
}
...
<div class="reports-wrapper">
    <div class="reports-section">
        <h2>✏️ Редагування замовлення</h2>
                        ...
                        <select name="product_id[]" required>
                            ...
                            <?php mysqli_data_seek($products, 0); while ($p = mysqli_fetch_assoc($products)) : ?>
                                <option value="<?= $p['id'] ?>" <?= $p['id'] == $item['product_id'] ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($p['name']) ?> (<?= $p['stock'] ?> шт.)
                                </option>
                            <?php endwhile; ?>
                        </select>
